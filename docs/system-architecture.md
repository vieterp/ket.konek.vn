...
| `PostingDocumentRegistry.codes_of_module(permission_module)` — registry chứng từ trả mã theo module chủ sở hữu, cho câu hỏi liên phân hệ "chứng từ này đã sinh ra phiếu của phân hệ kia chưa" mà không import phân hệ kia (ADR-028, `ket.posting` mở lần ba, cộng thêm). Chỉ trả **siêu dữ liệu đăng ký**; dữ liệu nghiệp vụ vẫn đi qua Protocol trong `kernel` | 8D | Mọi cặp phân hệ sinh chứng từ cho nhau (bán → kho, mua → kho, 9 lệnh sản xuất → kho) |
| `CostState.NOT_APPLICABLE = 3` — trạng thái giá **cuối** của dòng hàng giữ hộ: không có giá và sẽ không bao giờ có. Khác `PENDING` ở chỗ `lock_check` coi nó là đã chốt; mọi nơi hỏi "còn dòng chưa tính giá không" phải đọc `NOT IN (COSTED, NOT_APPLICABLE)` chứ không `<> COSTED` | 8D | Mọi lượt đọc `cost_state`; 8F báo cáo kho |
...
| Mã hàng · bảng giá | SYS (MasterData) | Trang chi tiết mã hàng (mức giá + bậc chiết khấu) và trang bảng giá (dòng giá) — 7H-2b — đọc/ghi thẳng ba bảng con của kernel (`/master/items/{id}/{prices,discount-tiers,units}`, `/master/price_lists/{id}/lines`), không BFF: một màn hình, một module. |
...
| Bán hàng | SAL | 1-1 với module. Router của `sales` phục vụ trực tiếp — lát 7H-2a thêm `GET /sales/invoices` (khách/hóa đơn hai nguồn/còn phải thu/dòng tổng theo tiền tệ) cùng khuôn lưới mua; form hỏi giá qua `pricing/quote-batch`; tab "việc còn thiếu" đi BFF, và bộ lọc `einvoice=missing` của lưới đọc **cùng hằng** với BFF. |
| Hóa đơn điện tử | EIV + INV | Một màn HĐĐT (7H-3): lưới đọc `GET /einvoices` dạng hàng lưới — tờ + phần chứng từ bán join ở tầng `api` (không BFF, RT-21); cột trạng thái với CQT gộp; wizard sai sót dựng câu hỏi từ `GET /error-flows`; hóa đơn đầu vào là tab cùng trang; panel outbox đọc `GET /einvoices/outbox`. Hồ sơ nhà cung cấp / đăng ký ký hiệu vẫn thuộc nhóm 07. |
| Sổ sách & Thuế | GLE + TAX + RPT | Sổ cái drill-down, khóa sổ là danh mục kiểm tra, tờ khai gộp. BFF `statements/financial-package` + `period-close/checklist` gọi cả 3. Form chứng từ nghiệp vụ khác có **khối đối trừ theo dòng** (7H-2b): router `gl_journal` thêm `GET /open-invoices` hỏi cho một dòng (TK + đối tác + bên + tiền tệ), chiều suy ở server bằng chính `posting.debt_lines.classify`. |
...
| `inventory_vouchers`, `inventory_voucher_lines` | Thân phiếu NK/XK/CK/LR/TD (một-một với `vouchers`, cột thủ kho `keeper_*` — 8D dùng) + dòng vật tư (số lượng theo ĐVT gõ và quy về đơn vị chính, giá vốn nếu biết, cặp TK bút toán kèm dòng; 8C-2: `is_product` — dòng thành phẩm của lắp ráp/tháo dỡ, ≤ 1 mỗi phiếu — và `allocation_ratio` chia giá trị khi tháo dỡ; 8D: `is_custodial` — dòng hàng nhận giữ hộ, không cặp TK, không giá) | 8A, 8C-2, 8D | `inventory` |
...
| `inventory_movements` | **Sổ kho** — append-only như `gl_postings`; khóa `(chi nhánh, kho, vật tư, lô)`, `sequence_in_day` duy nhất theo ngày (BR-STK-04), `cost_state` 0/1/2/**3** (3 = giữ hộ, 8D), RLS chi nhánh, trigger kỳ khóa (BR-STK-05); FK thật tới `items`/`warehouses`/`lots` để gộp danh mục dời theo. `is_custodial` (BR-STK-07): theo dõi số lượng, **không** vào giá trị tồn, engine tính giá lọc bỏ ở mọi câu | 8A, 8D | `inventory` |
...
| `inventory_count_sheets`, `inventory_count_sheet_lines` | Biên bản kiểm kê kho (FR-STK-030/031, U8) — chụp tồn sổ sách tại ngày kiểm kê (khóa duy nhất mang `is_custodial`: một khóa có cả hai loại hàng ra hai dòng đếm), số đếm thật, đơn giá phần thừa; "đã xử lý" = đã có `adjustment_receipt_id`/`adjustment_issue_id` (không có cột trạng thái, khuôn `cash_count_sheets`). RLS trên thân | 8D | `inventory` |
| `warehouse_book` | **Sổ kho của thủ kho** (SRS 17 §3.2) — số lượng vào/ra theo ngày THỦ KHO ghi, tách khỏi ngày hạch toán; phân hệ tắt (`warehouse.keeper.enabled` false) thì ghi thẳng lúc ghi sổ kế toán | 8A (rỗng), 8D (ghi) | `inventory` |
