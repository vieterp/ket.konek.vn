...
7. **Check toàn vẹn "sổ phụ công nợ khớp TK 131/331" vẫn chưa đăng ký** — bản thảo `posting/integrity/checks/arap_matches_control.sql` cố ý ngoài `CHECKS`, đầu tệp liệt kê từng điều kiện kèm bằng chứng. 7C-3 đóng #1 (bút toán ghi tay nay SINH dòng sổ phụ), 7C-4 đóng #2 ở vế chứng từ (khoản ứng trước của phiếu thu/chi + ngân hàng + chứng từ nghiệp vụ khác), #6 (lọc `ledger = 0`) và #8 (mốc năm tài chính). Ba cái còn mở, tất cả tìm thấy ở review 7C-4: **#7** khoản ứng trước ở SỐ DƯ ĐẦU KỲ không có dòng sổ phụ (`parsing.py` chỉ cấm dòng bên ngược *mang số chứng từ*, không cấm bản thân khoản ứng trước — BR-OPB-03 cho phép, và một bài test đang xanh là bằng chứng); **#3** `carry_forward` bỏ hóa đơn khi dư ròng của đối tác về 0, đúng ca có ứng trước; **#9** khối đối trừ của chứng từ tiền ở mức CHỨNG TỪ (một `partner_id`, không truyền `account_id`) trong khi sổ cái ghi theo đối tác TỪNG DÒNG. Đóng nốt ba cái rồi đăng ký ⇒ lát 7C-5.
