...
| `modules/*` | `cash_book/` (6B–6C, +`balance_service` 6E-1); `bank/`, `warehousing/treasurer/` (6C–6E-1); `general_ledger/journal/` (phase 4); `receivables/` (7A, +`guards.py` 7B); `purchase/` (7B, server; **chưa có UI client**); `sales/` vẫn rỗng — bộ định giá của nó sống ở `kernel/pricing` (7C-1) vì chiều mua cũng cần, module thật dựng ở 7C-2; còn lại chỉ có `contracts.py` rỗng — chỗ giữ sẵn cho phase 7C-2–9 | — |
...
Tiếp theo: **7C — `sales`** (giá + bậc chiết khấu, và **trả nốt check toàn vẹn 131/331** mà 7A phải hoãn). Cả 7B lẫn 7C gọi `ArApSubledger.record`/`remove` trong `after_post`/`after_unpost` — bất biến "dòng sổ phụ chỉ tồn tại khi chứng từ đã ghi sổ" là thứ giữ cho đường xóa an toàn.
...
9. **Module `purchase` (7B) chưa có UI client** — router `/api/v1/purchase/*` + `PartnerDebtGuard` chạy được qua API/test, màn hình nhập hóa đơn mua + chi phí mua hàng (và "Vẫn ghi sổ?" cho cảnh báo `warning.partner_debt`) thuộc lát UI kế tiếp.
