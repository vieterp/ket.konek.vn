...
Thao tác này **đặc quyền**: chạy bằng `konek_owner`, không phải vai trò runtime.
...
from konek.kernel.datasets.models import Dataset
...
from konek.kernel.security.grants import APP_ROLE, OWNER_ROLE
...
def find_alembic_config(explicit_path: Path | None = None) -> Config:
    ...
    Mặc định suy từ vị trí gói (`server/src/konek/...` → `server/`). Cách này
    ...
    `KONEK_ALEMBIC_INI_PATH` là đường thoát rõ ràng thay vì đoán mò.
    """
    path = explicit_path or Path(__file__).resolve().parents[4] / "alembic.ini"
    if not path.is_file():
        raise FileNotFoundError(
            f"Không tìm thấy alembic.ini tại {path}. Đặt KONEK_ALEMBIC_INI_PATH nếu chạy "
            "từ thư mục khác."
        )
...
def upgrade_dataset_schema(engine: Engine, schema: str, config: Config | None = None) -> None:
    ...
    Truyền sẵn `connection` để Alembic dùng lại engine `konek_owner` hiện có
    ...
    with engine.begin() as connection:
        ...
        # Chỉ SELECT: ghi phiên bản là việc của `konek_owner`.
