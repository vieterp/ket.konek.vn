...
* **`NUMERIC` ↔ `Decimal`.** psycopg 3 trả `NUMERIC` thành `Decimal` mặc định.
  ...
  `ket.kernel.money`.
* **Hai vai trò DB tách biệt** (RT-02/RT-06): `ket_app` chạy runtime,
  `ket_owner` chỉ chạy DDL/migration. Cùng một hàm dựng engine, khác DSN.
...
APPLICATION_NAME: Final[str] = "ket-app-server"
...
def create_app_engine(settings: Settings, *, application_name: str = APPLICATION_NAME) -> Engine:
    """Engine runtime (vai trò `ket_app`).
...
def create_owner_engine(settings: Settings) -> Engine:
    """Engine đặc quyền (vai trò `ket_owner`) — chỉ dùng cho DDL/migration.
