...
from ket.kernel.errors import BranchNotInScopeError, MasterDataNotFoundError
...
def _ensure_visible(record: MasterDataRow, branch_id: int | None, spec: CatalogSpec) -> None:
...
def _ensure_branch_in_scope(branch_id: int | None, authorized: AuthorizedRequest) -> None:
    ...
    không nhìn thấy nó sau đó (`_ensure_visible`), bản ghi ấy trở thành thứ chỉ
        ...
        **Có phân trang từ lát này**, dù mười bảy danh mục hiện tại đều nhỏ: hợp
        ...
        with unit_of_work(factory, authorized.scope) as session:
            ...
            if subtree_of is not None:
                ...
                _ensure_visible(service.get(subtree_of), branch_id, spec)
                page = service.subtree_of(
                    subtree_of, branch_id=branch_id, limit=limit, offset=offset
                )
            else:
                page = service.children_of(
                    parent_id, branch_id=branch_id, limit=limit, offset=offset
                )
